...
_JSON_TAIL = """
【最高优先级】你必须在回复**开头**输出一个 JSON 代码块（语言标记必须是 json），再写可选短附录。
前端页面只渲染 JSON 里的 `report` 卡片；Markdown 仅作附录。
**禁止**开场白；**禁止**只写 Markdown 不写完整 report；**禁止**把 JSON 放到文末导致截断丢失。
...
US_OUTPUT = """【输出顺序 · 必须遵守】
1) **先**输出完整 ```json ... ``` 代码块（含全部 report 卡片字段，见上方 schema）。
2) JSON 之后可用极短中文附录（每节约 2–4 行，可选），对应【1】～【8】提纲；**禁止**把长篇 Markdown 写在 JSON 前面。
3) 前端只认 JSON 里的 `report`；没有完整 JSON = 生成失败。
...
A_SHARE_OUTPUT = """【输出顺序 · 必须遵守】
1) **先**输出完整 ```json ... ``` 代码块（含全部 report 卡片字段）。
2) JSON 之后可用极短中文附录（可选）；**禁止**把长篇 Markdown 写在 JSON 前面。
3) 前端只认 JSON 里的 `report`。

可选附录提纲：【1】本周总判断（含 A股本土映射）【2】5 件事【3】评分【4】供应链【5】清单【6】情景【7】行动【8】转折点。
末尾一行免责声明即可。
"""
